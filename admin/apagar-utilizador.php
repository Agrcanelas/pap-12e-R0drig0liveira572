...
if ($id > 0) {
  // Check if user is used in servicos table as prestador
  $sql = "SELECT COUNT(*) AS count FROM servicos WHERE id_prestador = ?";
  ...
  if ($stmt) {
    ...
    if ($stmt->execute()) {
      ...
      if ($count > 0) {
        $message = 'Não é possível eliminar este utilizador porque está a ser utilizado em ' . $count . ' serviço(s).';
        $message_type = 'error';
      } else {
        // User is not used, proceed with deletion
        $sql_delete = "DELETE FROM utilizadores WHERE id_utilizador = ?";
        ...
        if ($stmt_delete) {
          ...
          if ($stmt_delete->execute()) {
            ...
            $message = 'Erro ao eliminar utilizador: ' . $stmt_delete->error;
            $message_type = 'error';
          }
          $stmt_delete->close();
        } else {
...
<head>
  <title>Eliminar Utilizador | Banco do Tempo</title>
  <!-- [Meta] -->
  <meta charset="utf-8">
          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <div class="page-header-title">
                <h2 class="mb-0">Eliminar Utilizador</h2>
              </div>
            </div>
          </div>
        </div>
