...
// Get service id from GET
$id = isset($_GET['id']) ? (int) $_GET['id'] : 0;

if ($id > 0) {
  // Check if service is used in transacoes table
  $sql = "SELECT COUNT(*) AS count FROM transacoes WHERE id_servico = ?";
  ...
  if ($stmt) {
    ...
    if ($stmt->execute()) {
      ...
      if ($count > 0) {
        $message = 'Não é possível eliminar este serviço porque está a ser utilizado em ' . $count . ' transação(ões).';
        $message_type = 'error';
      } else {
        // Service is not used, proceed with deletion
        $sql_delete = "DELETE FROM servicos WHERE id_servico = ?";
        ...
        if ($stmt_delete) {
          ...
          if ($stmt_delete->execute()) {
            ...
            $message = 'Erro ao eliminar serviço: ' . $stmt_delete->error;
            $message_type = 'error';
          }
          $stmt_delete->close();
        } else {
...
<head>
  <title>Eliminar Serviço | Banco do Tempo</title>
  <!-- [Meta] -->
  <meta charset="utf-8">
          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <div class="page-header-title">
                <h2 class="mb-0">Eliminar Serviço</h2>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card tbl-card">
            <div class="card-body">
              <div class="form-group mb-3">
                ...
                ?>
                <a href="servicos.php" class="btn btn-secondary mt-3">Voltar</a>
              </div>
            </div>
          </div>
        </div>
