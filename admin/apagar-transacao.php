...
if ($id > 0) {
  // Check if transaction estado is "concluido"
  $sql = "SELECT estado FROM transacoes WHERE id_transacao = ?";
  ...
  if ($stmt) {
    ...
    if ($stmt->execute()) {
      ...
      if ($estado !== 'concluido') {
        $message = 'Não é possível eliminar esta transação. Apenas transações com estado "Concluído" podem ser eliminadas.';
        $message_type = 'error';
      } else {
        // Transaction estado is "concluido", proceed with deletion
        $sql_delete = "DELETE FROM transacoes WHERE id_transacao = ?";
        ...
        if ($stmt_delete) {
          ...
          if ($stmt_delete->execute()) {
            ...
            $message = 'Erro ao eliminar transação: ' . $stmt_delete->error;
            $message_type = 'error';
          }
          $stmt_delete->close();
        } else {
...
<head>
  <title>Eliminar Transação | Banco do Tempo</title>
  <!-- [Meta] -->
  <meta charset="utf-8">
          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <div class="page-header-title">
                <h2 class="mb-0">Eliminar Transação</h2>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card tbl-card">
            <div class="card-body">
              <div class="form-group mb-3">
                ...
                ?>
                <a href="transacoes.php" class="btn btn-secondary mt-3">Voltar</a>
              </div>
            </div>
          </div>
        </div>
